    ...
    useEffect( () => {
        const getList = async () => {
            const req = (await Axios({
                ...
                url: "http://localhost:9000/importaciones/nota-recepcion/aceptar-documento/pendiente"
            })).data;
            setData(req);
        };
        getList();
    ...
    useEffect( () => {
        const aceptar = async (nNotaRecepcion) => { 
            (await Axios({
                ...
                data: { state: 'Aceptado' },
                withCredentials: true,
                url: "http://localhost:9000/importaciones/nota-recepcion/aceptar-documento/aceptado"
            }));
            setStateKeeper(''); 
            setReload(!reload);
        };
    ...
    useEffect( () => {
        const rechazar = async (nNotaRecepcion) => {
            (await Axios({
                ...
                withCredentials: true,
                url: "http://localhost:9000/importaciones/nota-recepcion/aceptar-documento/rechazado"
            }));
            setStateKeeper('');
            setReload(!reload);
        };
                    ...
                    <div className='dataTable container'>
                        <MaterialTable
                            ...
                            options= {{
                                ...
                                exportButton: true,
                                exportAllData: true,
                                exportFileName: 'Importaciones-NotaRecepcion-Pendientes'
                            }}
                            localization = {{
                                header:{
                                    actions:'Acciones'
